...
# flores200 lang codes choices
LANGUAGES: Final[List[Dict[str, str]]] = [
...
IMG_PATH: Final[str] = "img/"

# Convert the list of dictionaries to a dictionary with code as the key
codeLanguagesDict = {lang["code"]: lang["name"] for lang in LANGUAGES}
...
def translate(text: str, desiredLang: str):
    textLang = detect_language(text)
    print(textLang)
    detectedToReadableFormat = codeLanguagesDict[textLang]
    translator = pipeline(task='translation', model='facebook/nllb-200-distilled-600M')
    result = translator(text, src_lang=textLang, tgt_lang=desiredLang)
    return [detectedToReadableFormat, result[0]['translation_text']]


def extract_image_text(imgPath: str, desiredLang: str):
...
async def lang_autocomplete(interaction: discord.Interaction, current: str) -> List[app_commands.Choice[str]]:
    return [
        ...
        for lang in LANGUAGES if current.lower() in lang['name'].lower()
    ]
